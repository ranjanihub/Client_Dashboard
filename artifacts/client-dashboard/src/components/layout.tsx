...
export function TopNav({ onOpenMobileSidebar }: TopNavProps) {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 md:h-20 bg-white/95 backdrop-blur-md border-b border-[#eef1f6] px-3 sm:px-6 md:px-8 flex items-center justify-between shadow-xs gap-2">
      ...
      <div className="flex items-center gap-2">
        {onOpenMobileSidebar && (
          <button
            ...
            className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors md:hidden cursor-pointer"
        ...
        {/* Search */}
        <div className="flex-1 max-w-xs sm:max-w-md md:max-w-xl relative">
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 md:gap-4">
        <Link href="/sessions">
          <button type="button" className="p-2 sm:p-2.5 rounded-full text-slate-500 hover:bg-slate-100 transition-colors cursor-pointer" title="View Schedule Calendar">
            <Calendar className="w-4 sm:w-5 h-4 sm:h-5 text-slate-600" aria-hidden="true" />
          </button>
        </Link>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <div className="relative">
              <button type="button" className="p-2 sm:p-2.5 rounded-full text-slate-500 hover:bg-slate-100 transition-colors relative cursor-pointer focus:outline-none" title="Notifications">
                <Bell className="w-4 sm:w-5 h-4 sm:h-5 text-slate-600" aria-hidden="true" />
                {unreadCount > 0 && (
          ...
          <PopoverContent align="end" className="w-[380px] sm:w-[410px] max-w-[92vw] p-0 shadow-2xl rounded-3xl border border-slate-200 overflow-hidden font-sans">
            {/* Popover Header */}
        ...
        <Link href="/messages">
          <button type="button" className="inline-flex items-center gap-2 rounded-full bg-[#5e2be2] hover:bg-[#4f28d9] text-white font-bold text-xs sm:text-sm h-9 sm:h-10 px-3.5 sm:px-5 shadow-md shadow-[#5e2be2]/25 transition-all hover:scale-105 active:scale-95 cursor-pointer">
            <MessageSquare className="w-3.5 sm:w-4 h-3.5 sm:h-4" aria-hidden="true" />
            <span className="hidden sm:inline">Messages</span>
          </button>
        </Link>

        {/* User profile link */}
        <Link href="/profile" className="flex items-center gap-3 pl-1 sm:pl-2 border-l border-slate-200">
